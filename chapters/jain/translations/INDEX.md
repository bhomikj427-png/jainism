...
| 5 | Bhaktāmar — remaining verses | `devotional/bhaktamar/01-bhaktamar.md` (continuing same file) | **in-progress — v.1–20 done, v.21 → end pending** | v.7–20 drafted: praise of the Jina's radiance/virtues via moon/sun/lamp/gem comparisons. New source pair anchors this stretch: jainsquare's paginated "1–48 stotras" series (numbering independently cross-checked against bhaktinidhi.com's full-Sanskrit 48-verse text) + a named translator, Bhagwan Das Jaini, via cs.colostate.edu/~malaiya. v.12's *paramāṇu* (atom) got full treatment incl. a sourced Modern-comparison box (atom/"God particle" conflation, explicitly NOT-equivalent per scholarly pushback) and a cross-reference to the TS Adhyāya 5 doctrinal track's atom material. The Vijay K. Jain archive.org source used for v.1–6 turned out to be a preview excerpt only (verses 1–6, no more) — resolved the prior session's open "is the next verse really v.7" caution: it wasn't v.7, it was a later miracle-verse that happened to be in the preview file; v.7 itself is now independently confirmed. |
