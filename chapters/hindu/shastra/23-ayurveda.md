...
**A third authority, and a gap in the graph.** **Vāgbhaṭa's *Aṣṭāṅga Hṛdayam*** is frequently cited alongside the two Saṃhitās as the third classical authority — and ***vagbhata* is not a written concept node.** It sits unwritten in `progress.md`'s batch queue, along with `dhatu`/`mala`. So this chapter covers a tradition whose third pillar the corpus has not yet built, and says so rather than writing around the absence.
