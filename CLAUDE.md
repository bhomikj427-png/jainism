...
- For "what links to X" or any whole-graph question, use **grep / the graph script** — never load every file. For "does X exist / what's its status / how many links / what's its devanagari," grep **`MANIFEST.tsv`** (one line per concept) — do **not** load `index.md` wholesale or open the concept file.
...
1. **Grep `MANIFEST.tsv` for the key (`grep -P "^<key>" MANIFEST.tsv`).** A row → the concept exists; do **not** create — add typed links to it instead. The prefix grep also surfaces the **tradition-suffix family** (`ahimsa`, `ahimsa-vedic`, `ahimsa-buddhist`) and each member's devanagari, so you don't mint a third copy of a word that's already split. (Equivalent fallback: `Glob concepts/<key>*.md`.)
