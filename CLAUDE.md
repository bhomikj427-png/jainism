...
- **Directional types** — `is-a-type-of`, `part-of`, `formalizes`, `expressed-by`, `aggregates-into`, `historically-influenced-by` — encode a real asymmetry (child→parent, part→whole, later-thinker→source, etc.). Store **only the forward direction**; the backlink is computed with grep, never hand-maintained. A directional edge present in **both** directions on the same pair is a **defect** — it asserts two contradictory structural claims (A is part of B *and* B is part of A; A was influenced by B *and* B by A). The deterministic audit flags these; resolve by keeping the one chronologically/structurally correct direction. ⚠️ **This means the *same* directional type in both directions.** **Complementary inverse pairs are correct and are the corpus's established idiom** — `A part-of B` alongside `B expressed-by A`, `A is-a-type-of B` alongside `B expressed-by A`, `A aggregates-into B` alongside `B aggregates-from A`. These state one asymmetric relation from each end and do **not** contradict each other; as of Batch 43 there are ~75 such pairs and **zero** same-type defects. Do not "repair" them.
  - ⚠️ **Direction rule for `expressed-by` (settled Batch 46).** It points **person → doctrine/work**, never doctrine → person — the agent points at what they expressed. This is the idiom already committed in `udayana.md` (`expressed-by: jati`, `expressed-by: pramana-nyaya`). Batch 46 introduced **three** same-type bidirectional `expressed-by` defects by writing it both ways in one batch; the audit caught all three. A broader thing may be `expressed-by` a narrower vehicle (`advaita-vedanta expressed-by vimuktatman`, `navya-nyaya expressed-by avacchedaka`), but a **doctrine never points at its author**. ⚠️ **Written into the charter only after Batch 47 hit it too** — that batch caught and fixed **four** doctrine→person `expressed-by` edges in `vyakarana.md`, working from a convention that until then lived only in a run-log. **Two consecutive batches making the same error is what promotes a convention to a rule.**
...
- `progress.md` — the **work-queue + most-recent run-log only**: the active batch and each concept's state (`pending | done | blocked`), plus the latest run-summary. Kept **bounded** (loaded on every startup); `build_graph.py` nags if it exceeds ~60 KB. This is how a relaunched session knows the plan. ⚠️ The startup set is **three** files, not one: `CLAUDE.md` + `progress.md` + `chapters/INDEX.md`. `build_graph.py` nags on `chapters/INDEX.md` past ~60 KB (shard the concept→chapter map per origin-folder with a dispatcher) and prints the combined budget (soft limit 120 KB).
...
- `chapters/` — the **human-readable teaching layer**: long-form prose reading-views of the material, one file per **chapter** (numbered `01`, `02`, …), grouped by origin into subfolders (`jain/`, `cross-tradition/`, `comparanda/`, `hindu/`, `buddhist/`), and tracked in `chapters/INDEX.md` (the **chapter list + roadmap**; small, loaded at startup) with the 340-row **concept→chapter lookup table** split out into `chapters/coverage.md` (**not** loaded at startup — grep it, exactly as `MANIFEST.tsv` is grepped instead of loading `index.md`). **Chapters are reading views, NOT graph nodes** — `build_graph.py` does **not** scan `chapters/`. ⚠️ A **"chapter" (a file here) is NOT the same as a "batch"** (a concept work-queue unit in `progress.md`); when the user says "chapter," they mean a file in `chapters/` — consult `chapters/INDEX.md` for the next chapter number, never `progress.md`'s "Suggested Batch."
- `graph/build_graph.py` — deterministic script: scans `concepts/*.md`, parses front-matter + `## Links`, builds nodes (**size = link count, colour = tradition**) and edges (**style = link type**: solid = structural, dashed = honest cross-tradition, dotted = conflated-NOT-equivalent). Renders `graph/graph.svg` via Graphviz — **untracked** (`.gitignore`), because `fdp` relays the graph out on every run even with a fixed seed, so it rewrote ~12k lines / 1.1 MB per commit for no information; render it on demand. The **tracked, byte-stable** artifacts are `graph/graph.dot` and `graph/graph.html`. It also and emits a self-contained `graph/graph.html` — an offline force-directed view built on the vendored `graph/vendor/force-graph.min.js`, with the same three edge layers drawn solid / dashed / dotted. Idempotent — always regenerated. It also runs the **structural audit** (dangling stubs, orphans, same-type bidirectional directional edges, forbidden hier+similarity combos) *and* a **conformance audit** (link types, `status`, `confidence` and required front-matter checked against the §3/§5 vocabularies), warns if `graph.svg` is **stale** relative to the corpus, and prints the startup-set size budget. If `dot` is not on PATH it is looked up in the standard install locations before giving up.
- `graph/check_all.py` — **the single gate for §8/§9.** Runs the three checks below in order, always running all three so one command shows the whole picture, and exits non-zero if any fails. ⚠️ `build_graph.py`'s audit printed “DEFECTS PRESENT” but `main()` dropped the verdict, so the process always exited 0 — the audit was advisory, never a gate. Fixed; it now exits 1 on any structural or conformance defect (a missing SVG render or an oversized startup set stay advisory).
- `graph/find_duplicates.py` — deterministic **duplicate-concept detector** (read-only). Reports six classes: `[DEVANAGARI]` same `term_devanagari` across files (same Sanskrit word — must be a typed tradition-split, not an accident); `[IAST]` same `term_iast` front-matter under two filenames (hard collision); `[TRANSLIT]` distinct keys that fold to one canonical form — transliteration twins (`sunyata`/`shunyata`); `[PHANTOM]` a `## Links` target with no file but within edit-distance 1 of a real file (a typo spawning a stray node); plus two report-only classes — `[SPLIT]` (keys differing only by a tradition suffix — an expected typed split; verify the typed edge exists) and `[MANIFEST]` (is `MANIFEST.tsv` in sync with `concepts/`). Exit 1 on any IAST/TRANSLIT/PHANTOM group. Run before creating a concept and as part of the §8 pre-commit check.
- `graph/check_chapters.py` — deterministic **teaching-layer checker** (read-only). Joins `chapters/coverage.md`'s concept→chapter map (and `chapters/INDEX.md`'s chapter list) to `concepts/` and reports `[UNRESOLVED]` (a map row naming no concept file), `[UNCOVERED]` (a written concept no chapter teaches), `[DUPLICATE]` (a concept with >1 row — flagged **CONFLICT** when the rows disagree on the primary chapter) and `[MISSINGFILE]` (a chapter file named but absent). Exit 1 on any. This exists because "chapter coverage N/N" was the one structural claim in the repo asserted **by hand** rather than proven — and when first checked it was wrong. Run it with the §8 pre-commit check and at end-of-batch.
...
- After any bulk work, run **`python graph/check_all.py`** — the single gate: it runs `build_graph.py`, `find_duplicates.py` and `check_chapters.py`, always all three, and exits non-zero if any fails. A non-zero exit is a defect to resolve under §10 before pushing.
...
When the batch is complete (or you hit the token wall): **run `python graph/check_all.py`** (which runs `build_graph.py` (regenerates `graph.*`, `index.md`, and `MANIFEST.tsv`, and runs the structural + conformance audits, the graph.svg staleness check and the startup-set size nags) `find_duplicates.py` and `check_chapters.py` — the last of which proves the chapter-coverage figure instead of asserting it), then append a run-summary to `progress.md` — concepts completed (count + list with status), anything `blocked`/`needs-opus-review`, and a suggested next batch (names only, unwritten) — final commit, then **halt**.
