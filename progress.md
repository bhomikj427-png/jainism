...
> 2. **Chapters** → `chapters/*.md`, the human-readable **teaching layer** (prose reading-views, NOT graph nodes; `build_graph.py` ignores them). Indexed in **`chapters/INDEX.md`**. Latest: **Ch 24 drafted — and the chapter roadmap is now EXHAUSTED**: every concept written in `concepts/` has a primary chapter home. **→ Before starting Ch 25, re-derive the roadmap** (diff `concepts/*.md` against the "Full concept → chapter index" in `chapters/INDEX.md`); if that diff is empty, the next unit of work is a **concept batch here**, not a chapter. NOTE: `hindu/` now has a **second level** (`darsana/`, `devotional/`, `scripture/`) — see `chapters/INDEX.md` header for what goes where.
>
