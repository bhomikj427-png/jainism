---
term_iast: mokṣa
term_devanagari: मोक्ष
tradition: Jain
source_text: Tattvārtha Sūtra (Umāsvāti/Umāsvāmī), Chapter 10
status: converged
confidence: medium
---

## Gloss / Divergence map

Mokṣa is **liberation** — the complete annihilation of all karma from the soul (jīva), ending the cycle of rebirth (saṃsāra). The *Tattvārthasūtra*'s final chapter defines it directly:

**TS 10.2**: "Owing to the absence of the cause of bondage and with the functioning of the dissociation of karmas, the annihilation of all karmas is liberation." (Cited in Wikipedia "Mokṣa (Jainism)," sourcing the TS.)

The TS is itself sometimes called the *Mokṣaśāstra* (Scripture on Liberation) — mokṣa is the entire text's telos.

**The liberated soul — siddha:** At mokṣa the soul attains its "true and pristine nature": four infinite qualities (*ananta-catuṣṭaya*), traditionally enumerated:

| quality | Sanskrit | description |
|---|---|---|
| Infinite knowledge | ananta-jñāna | complete, unobstructed omniscience |
| Infinite perception/faith | ananta-darśana | unlimited insight/apprehension |
| Infinite bliss | ananta-sukha | unobstructed bliss (no vedanīya karma remains) |
| Infinite energy | ananta-vīrya | unlimited capacity (no antarāya karma remains) |

(Source: Wikipedia "Mokṣa (Jainism)," citing *Puruṣārthasiddhyupāya*; the four-infinite formula is standard across Jain sources.)

**Where the siddha goes — lokāgra:** Upon final liberation (physical death of the last body after gaining *kevala-jñāna*), the soul rises automatically to *Siddhashila* — the summit (*agra*) of the Jain cosmos (*loka*) — and remains there eternally. The upward motion is natural: freed from karma's downward-binding weight, the intrinsically weightless soul rises like a flame. (Wikipedia "Mokṣa (Jainism).")

**The process leading to mokṣa** (completing the soteriology arc from karma.md):
1. *Saṃvara* — stopping new āsrava (karmic influx) by ethical restraint
2. *Nirjarā* — actively shedding existing karma through austerity (*tapas*)
3. At complete karma annihilation: *kevala-jñāna* (omniscience) → followed by *mokṣa* at death
4. The soul then dwells at lokāgra as a *siddha* forever

**The three jewels (*tri-ratna*)** as the path: Right faith (*samyak-darśana*) + Right knowledge (*samyak-jñāna*) + Right conduct (*samyak-cāritra*) — the TS's own summary of the path to liberation. TS 1.1: *samyag-darśana-jñāna-cāritrāṇi mokṣamārgaḥ* ("Right faith, knowledge, and conduct together are the path to liberation").

**Critical distinction — Jain mokṣa vs. Advaita Vedānta mokṣa:**
In Advaita Vedānta, mokṣa = recognition that individual ātman is identical to universal Brahman; individual identity merges into the absolute. In Jainism, each siddha remains a **distinct individual consciousness** at lokāgra — infinite in knowledge but not merged with any other soul or universal Being. The plurality of ātmans is permanent; there is no universal Ātman to merge into. This structural distinction follows directly from the jīva.md pluralism. (Not sourced from a fetched text this run — listed as a comparison to verify.)

## Reasoning across signals

**Signal 1 — Wikipedia "Mokṣa (Jainism)":** TS 10.2 cited with English translation; siddha's qualities (four infinites) from *Puruṣārthasiddhyupāya*; lokāgra location. Fetched directly. Wikipedia-level provenance with TS verse reference.

**Signal 2 — Search result aggregation (encyclopediaofjainism, jainworld, encyclopedia.pub):** Confirms Chapter 10 covers mokṣa and siddha; three jewels as path; karma annihilation definition. Consistent with Signal 1. Derivative.

**Signal 3 — Karma file cross-check (concepts/karma.md):** The soteriology arc (āsrava→bandha→saṃvara→nirjarā→mokṣa) is internally consistent; mokṣa is the completion of that arc.

**Independence assessment:** Signal 1 provides the TS 10.2 verse citation. Signal 2 is derivative but consistent. TS 10.2 not fetched from the critical edition directly (Wikipedia citation only). The four-infinite formula is from *Puruṣārthasiddhyupāya* (a different Jain text, not the TS itself) — correctly noted as secondary text. Confidence medium.

**TS 1.1 note:** The tri-ratna formula is TS 1.1 (*samyag-darśana-jñāna-cāritrāṇi mokṣamārgaḥ*) — the first verse of the TS, which opens the text with its stated purpose. This verse number is standard and universally cited; not independently fetched from critical edition this run.

## Sources

1. Wikipedia, "Mokṣa (Jainism)," accessed June 2026. https://en.wikipedia.org/wiki/Moksha_(Jainism) — TS 10.2 citation; siddha qualities; lokāgra location. Medium provenance.

2. Nathmal Tatia (tr.), *That Which Is: Tattvārthasūtra*, HarperCollins/International Sacred Literature Trust, 1994. ISBN 0060689854. — anchor text; Chapter 10 not fetched.

3. Vijay K. Jain (tr. & ed.), *Tattvartha Sutra* (with Pūjyapāda's Sarvārthasiddhi), Motilal Banarsidass, 2018. ISBN 9788193272626. — Chapter 10 not fetched.

4. **Not yet fetched (needed for confidence upgrade):** TS 10.1–10.2 text from either critical edition directly; Puruṣārthasiddhyupāya for four-infinite formula.

## Links

- part-of: jiva | mokṣa is the telos of jīva; the liberated siddha is the jīva in its natural, karma-free state
- part-of: karma | mokṣa = total annihilation of all karma; completes the saṃvara→nirjarā→mokṣa arc
- shares-vocabulary-with: moksha-advaita | same Sanskrit term (mokṣa) used in Advaita Vedānta for liberation; but Advaita's mokṣa = ātman merging into Brahman; Jain's mokṣa = distinct individual soul at lokāgra — pluralism vs. monism, different structure
- expressed-by: siddha | siddha (the liberated soul at Siddhāśilā) is the state mokṣa produces; mokṣa is the event of final karma-annihilation, siddha is the resultant being
- expressed-by: charitra | cāritra (right conduct; TS 1.1 third jewel) is the ethical-behavioral path that produces mokṣa; without cāritra, samyag-darśana and samyag-jñāna cannot achieve liberation alone
